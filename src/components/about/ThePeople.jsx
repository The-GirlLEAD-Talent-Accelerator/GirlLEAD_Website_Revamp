...
export default function ThePeople() {
  ...
  return (
    <section style={{ fontFamily: "'DM Sans', sans-serif", background: "#014451" }} className="py-20 px-4 sm:px-6 lg:px-8">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap');
        .people-title { font-size: 2rem; font-weight: 700; color: #ffffff; margin-bottom: 4px; }
        .people-subtitle { font-size: 14px; color: #a0c4c0; }
        .people-tabs { display: flex; justify-content: center; border-bottom: 2px solid #025f70; margin-bottom: 36px; }
        .people-tab-btn { padding: 10px 28px; font-size: 14px; font-weight: 600; color: #7ab8b2; background: transparent; border: none; border-bottom: 3px solid transparent; margin-bottom: -2px; cursor: pointer; transition: color 0.2s, border-color 0.2s; font-family: 'DM Sans', sans-serif; }
        .people-tab-btn:hover { color: #e0f4f2; }
        .people-tab-btn.active { color: #ffffff; border-bottom-color: #00c2a8; }
        .people-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap: 20px; max-width: 860px; margin: 0 auto; }
        .people-card { border-radius: 14px; overflow: hidden; background: #ffffff; box-shadow: 0 2px 12px rgba(0,0,0,0.2); transition: transform 0.25s ease, box-shadow 0.25s ease; }
        .people-card:hover { transform: translateY(-5px); box-shadow: 0 14px 36px rgba(0,0,0,0.3); }
        .people-card-imgwrap { aspect-ratio: 1/1; overflow: hidden; }
        .people-card-imgwrap img { width: 100%; height: 100%; object-fit: cover; object-position: top center; transition: transform 0.3s ease; }
        .people-card:hover .people-card-imgwrap img { transform: scale(1.05); }
        .people-card-info { padding: 14px; }
        .people-card-name { font-size: 14px; font-weight: 700; color: #1a2e2a; margin: 0 0 2px; }
        .people-card-role { font-size: 12px; color: #3a7a6a; margin: 0 0 10px; }
        .people-card-socials { display: flex; gap: 7px; flex-wrap: wrap; }
        .people-social-icon { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 50%; background: #f0f5f3; color: #3a7a6a; font-size: 13px; text-decoration: none; transition: background 0.2s, color 0.2s; }
        .people-social-icon:hover { background: #014451; color: #fff; }
      `}</style>
